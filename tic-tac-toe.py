...
def symbol_player():
 ...
 if player == 'X' and computer == False:
  XO_list.append('X')
  print(XO_list)
  print(checklist)
  print(checklist2)
 ...
 elif player == 'X' and computer == True:
  XO_list.append('X')
  print(XO_list)
  print(checklist)
  print(checklist2)
...
button0 = tk.Button(game, text ="", bg='grey', width=10, height=5,  command=lambda:[click0(),symbol_player(), test_condition(), auto()])
...
button2 = tk.Button(game, text ="",bg='grey', width=10, height=5,  command=lambda:[click2(),symbol_player(), test_condition(), auto()])
button2.place(x=300, y=100)

button3 = tk.Button(game, text ="",bg='grey', width=10, height=5, command=lambda:[click3(),symbol_player(), test_condition(), auto()])
button3.place(x=100, y=200)

button4 = tk.Button(game, text ="",bg='grey', width=10, height=5,command=lambda:[click4(),symbol_player(), test_condition(), auto()])
button4.place(x=200, y=200)

button5 = tk.Button(game, text ="",bg='grey', width=10, height=5, command=lambda:[click5(),symbol_player(), test_condition(), auto()])
button5.place(x=300, y=200)

button6 = tk.Button(game, text ="",bg='grey', width=10, height=5, command=lambda:[click6(),symbol_player(), test_condition(), auto()])
button6.place(x=100, y=300)

button7 = tk.Button(game, text ="",bg='grey', width=10, height=5, command=lambda:[click7(),symbol_player(), test_condition(), auto()])
button7.place(x=200, y=300)

button8 = tk.Button(game, text ="",bg='grey', width=10, height=5, command=lambda:[click8(),symbol_player(), test_condition(), auto()])
...
menu1.add_command(label="Jouer contre un autre joueur", command=pvp)
menu1.add_command(label="Jouer contre l'ordinateur", command=lambda:[ia_system(), ia1()])
menu1.add_command(label="Niveau de l'ordinateur")
menu1.add_command(label="Recommencer", command=play_again)
menu1.add_separator()
...
menu3.add_command(label="Règles du jeu", command=messagebox.showinfo("Règles", "Chaque joueur joue à son tour en cliquant sur l'une des cases libres, le premier à aligner 3 symboles de manière horizontale, verticale ou diagonale remporte la partie"))
